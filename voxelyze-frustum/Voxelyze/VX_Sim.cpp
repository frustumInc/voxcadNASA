...
void CVX_Sim::ClearAll(void) //Reset all initialized variables
{
	...
	CurTime = (vfloat)0.0;
	std::fill_n(TimeOfLastNeuralUpdate,100,-999.0);
	CurStepCount = 0;
	DtFrozen = false;
...
#pragma omp parallel for
			...
			// for (int i=0; i<iT; i++)
			// {
			// 	// if (GetNeuralOutputOfIndex(0) > 0.5)
			// 	// {
			// 	// 	// VoxArray[i].SetElectricallyActiveNew(true);
			// 	// 	// VoxArray[i].SetRepolarizationStartTime(CurTime);
			// 	// 	// std::cout << "here: " << std::endl;
			// 	// }
			// 	// else
			// 	// {
			// 	// 	VoxArray[i].SetElectricallyActiveNew(false);
			// 	// }
			// 	// std::cout << "VoxArray[i].GetElectricallyActiveOld(): " << VoxArray[i].GetCurScale().x  << std::endl;
			// }

						// if (fabs(CurTime - TimeOfLastNeuralUpdate) > pEnv->GetNeuralUpdatePeriod())
			// {
			// 	TimeOfLastNeuralUpdate = CurTime; 
			// 	// UPDATE NEURAL NETWORK
			// 	std::cout << pEnv->pObj->GetSynapseWeight(0,0) << std::endl;

			// 	SetNeuralOutputOfIndex(0,GetSensorValueOfIndex(0));
			// }
			float outputValue;

			// for (int i=0; i<pEnv->pObj->GetNumSensors()+pEnv->pObj->GetNumHidden()+pEnv->pObj->GetNumMuscles(); i++)
			// {
			// 	std::cout << "last neural update of node "<< i <<": " <<  TimeOfLastNeuralUpdate[i] << std::endl;
			// }

			float firstTime = 0.001;
			

			for (int thisSensor = 0; thisSensor<pEnv->pObj->GetNumPacemaker(); thisSensor++)
			{
				if (pEnv->GetCurTemp() > pEnv->GetTempBase())
				{
					if (GetNeuralOutputOfIndex(thisSensor)==-1 or CurTime < firstTime)
					{
						sensorTraceFile << "pacemaker:" << thisSensor << ":+1:" << CurTime << std::endl;
					}
					SetNeuralOutputOfIndex(thisSensor,1.0);
				}
				else
				{
					if (GetNeuralOutputOfIndex(thisSensor)==1 or CurTime < firstTime)
				...
				// std::cout << "Neural Output of Pacemaker" << i << "at" << CurTime << ": " << GetNeuralOutputOfIndex(i) << std::endl;
				
			}

			for (int thisSensor=pEnv->pObj->GetNumPacemaker(); thisSensor<pEnv->pObj->GetNumSensors(); thisSensor++)
			{
				if (fabs(CurTime - TimeOfLastNeuralUpdate[thisSensor]) > pEnv->pObj->GetNodeTimeConstant(thisSensor)/100.0)
				{
					TimeOfLastNeuralUpdate[thisSensor] = CurTime;
					int i;
					int lastSensorValue = GetNeuralOutputOfIndex(thisSensor);
					for ( i=0; i<iT; i++)
					{
						if (VoxArray[i].GetSensorType()==thisSensor)
						{
							if (VoxArray[i].GetCurGroundPenetration()>0)
							// if (true)
							{
								if (lastSensorValue==-1 or CurTime < firstTime)
								{
									sensorTraceFile << "sensor:" << thisSensor << ":+1:" << CurTime << std::endl;
								}
								SetNeuralOutputOfIndex(thisSensor,1.0);
								break;  // double check is breaking correctly, and all sensors are still checked at least once
							}
							else
							{
								SetNeuralOutputOfIndex(thisSensor,-1.0);
					...
					if (i == iT and (lastSensorValue==1 or CurTime < firstTime))
					{
						sensorTraceFile << "sensor:" << thisSensor << ":-1:" << CurTime << std::endl;
					}
					// std::cout << "Neural Output of Sensor " << thisSensor << " at " << CurTime << ": " << GetNeuralOutputOfIndex(thisSensor) << std::endl;
					
				}
			}

			for (int i=pEnv->pObj->GetNumSensors(); i<pEnv->pObj->GetNumSensors()+pEnv->pObj->GetNumHidden(); i++)
			{
				if (fabs(CurTime - TimeOfLastNeuralUpdate[i]) > pEnv->pObj->GetNodeTimeConstant(i)/100.0)
				{
					TimeOfLastNeuralUpdate[i] = CurTime;
					outputValue = 0.0;

					for (int j=0; j<pEnv->pObj->GetNumSensors()+pEnv->pObj->GetNumHidden(); j++)
					{
						outputValue+=pEnv->pObj->GetSynapseWeight(j,i)*GetNeuralOutputOfIndex(j)-pEnv->pObj->GetNodeBias(j);
					}
					// outputValue = 1.0/(1.0+exp(-outputValue));
					outputValue = tanh(outputValue);

					
					// std::cout << "Neural Output of Hidden " << i << " at " << CurTime << ": " << GetNeuralOutputOfIndex(i) << std::endl;
					if (outputValue <= 0 and (GetNeuralOutputOfIndex(i)>0 or CurTime < firstTime))
					{
						sensorTraceFile << "hidden:" << i << ":" << "-1" << ":" << CurTime << std::endl;
					}
					else if (outputValue > 0 and (GetNeuralOutputOfIndex(i)<=0 or CurTime < firstTime))
					{
						sensorTraceFile << "hidden:" << i << ":" << "+1" << ":" << CurTime << std::endl;
					}
					SetNeuralOutputOfIndex(i, outputValue);
				}
			}

			for (int i=pEnv->pObj->GetNumSensors()+pEnv->pObj->GetNumHidden(); i<pEnv->pObj->GetNumSensors()+pEnv->pObj->GetNumHidden()+pEnv->pObj->GetNumMuscles(); i++)
			{
				if (fabs(CurTime - TimeOfLastNeuralUpdate[i]) > pEnv->pObj->GetNodeTimeConstant(i)/100.0)
				{
					TimeOfLastNeuralUpdate[i] = CurTime;
					...
					for (int j=pEnv->pObj->GetNumSensors(); j<pEnv->pObj->GetNumSensors()+pEnv->pObj->GetNumHidden(); j++)
					{
						outputValue+=pEnv->pObj->GetSynapseWeight(j,i)*GetNeuralOutputOfIndex(j)-pEnv->pObj->GetNodeBias(j);
					}
					// outputValue = 1.0/(1.0+exp(-outputValue));
					...
					if (outputValue <= 0 and (GetNeuralOutputOfIndex(i)>0 or CurTime < firstTime))
					{
						sensorTraceFile << "muscle:" << i << ":" << "-1" << ":" << CurTime << std::endl;
					}
					else if (outputValue > 0 and (GetNeuralOutputOfIndex(i)<=0 or CurTime < firstTime))
					{
						sensorTraceFile << "muscle:" << i << ":" << "+1" << ":" << CurTime << std::endl;
					...
					SetNeuralOutputOfIndex(i, outputValue);
					// std::cout << "Neural Output of Muscle " << i << " at " << CurTime << ": " << GetNeuralOutputOfIndex(i) << std::endl;
				}
			}

			// if (random() < float(RAND_MAX)/100)
			// {
			// 	std::cout << "Time: " << CurTime << std::endl;
			// 	for (int i=0; i<pEnv->pObj->GetNumSensors(); i++)
			// 	{
			// 		std::cout << "Sensor " << i << ": " << GetNeuralOutputOfIndex(i) << std::endl;
			// 	}

			// 	for (int i=pEnv->pObj->GetNumSensors(); i<pEnv->pObj->GetNumSensors()+pEnv->pObj->GetNumHidden(); i++)
			// 	{
			// 		std::cout << "Hidden " << i << ": " << GetNeuralOutputOfIndex(i) << std::endl;
			// 	}

			// 	for (int i=pEnv->pObj->GetNumSensors()+pEnv->pObj->GetNumHidden(); i<pEnv->pObj->GetNumSensors()+pEnv->pObj->GetNumHidden()+pEnv->pObj->GetNumMuscles(); i++)
			// 	{
			// 		std::cout << "Muscle " << i << ": " << GetNeuralOutputOfIndex(i) << std::endl;
			// 	}
			// }
...
float CVX_Sim::GetFloorHeight(float x, float y)
{
	// float floorParams[] = {0.002,0.003,0.001,0.002,0.001,0.003,0.003,0.005};
	float floorParams[] = {200,300,100,200,100,300,300,500};

	float ans;
	ans = sin(floorParams[0]*x + floorParams[1]*y) + (-1)*sin(floorParams[2]*x - floorParams[3]*y) + cos(floorParams[4]*x + floorParams[5]*y) + (-1)*cos(floorParams[6]*x - floorParams[7]*y);

	// if (random() < RAND_MAX/10000.0){
		// std::cout << "floor value at (" << x << ", " << y << "): " << ans << std::endl;
	// }
