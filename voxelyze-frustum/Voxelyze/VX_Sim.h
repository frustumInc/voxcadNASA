...
//! The integrator to be used to advance the simulation each timestep.
enum IntegrationType {
	I_EULER, //!< Euler integration. This should be suitable for nearly all situations. 
	I_RK4, //!< Runge-Kutta integration. Slower than Euler with benefits only when stiffnesses change quickly.
	I_VERLET //!< Verlet integration. Similar performance to Euler but without explicit momentum.
};
...
enum StopCondition {
	...
	SC_MIN_MAXMOVE //!<runs until maximum voxel displacement/timestep is below a threshhold
};
...
const char	CALCSTAT_NONE = 0x00, CALCSTAT_ALL = 0xff, CALCSTAT_COM = 0x01, CALCSTAT_DISP = 0x02, CALCSTAT_VEL = 0x04, CALCSTAT_KINE = 0x08, CALCSTAT_STRAINE = 0x10, CALCSTAT_ENGSTRAIN = 0x20, CALCSTAT_ENGSTRESS = 0x40;
struct SimState { //Information about current simulation state:
	void Clear() {CurCM = TotalObjDisp = Vec3D<>(0,0,0); NormObjDisp = MaxVoxDisp = MaxVoxVel = MaxVoxKinE = MaxBondStrain = MaxBondStress = MaxBondStrainE = TotalObjKineticE = TotalObjStrainE = 0.0;}
	Vec3D<> CurCM;
	Vec3D<> TotalObjDisp; //a vector total of the magnitude of displacements
	vfloat NormObjDisp; //reduced to a scalar (magnitude) 
	vfloat MaxVoxDisp, MaxVoxVel, MaxVoxKinE, MaxBondStrain, MaxBondStress, MaxBondStrainE;
...
//	std::ofstream out;
	...
	int NumVox(void) const; //!< Returns the number of voxels in the simulation.
	std::vector<CVXS_Bond> BondArray; //!< The main array of bonds.
	bool UpdateAllBondPointers(); //updates all pointers into the VoxArray (call if reallocated!)
	int NumBond(void) const; //!< Returns the number of bonds in the simulation.
	...
	bool CreateBond(BondType BondTypeIn, int SIndex1In, int SIndex2In, bool PermIn = false, int* pBondIndexOut = NULL, bool LinkBond = true); //!< Creates a new bond between two voxels. 
	bool UpdateBond(int BondIndex, int NewSIndex1In, int NewSIndex2In, bool LinkBond = true);
	void DeleteTmpBonds(void); //!< Deletes all non-permanent bonds.
	...
	vfloat TimeOfLastNeuralUpdate[100]; 
	bool sensorTrace;
	std::string traceFileName;
	std::ofstream sensorTraceFile;

	//Simulator features:
	//Self collision:
	void EnableSelfCollision(bool Enabled) {if (SelfColEnabled != Enabled){SelfColEnabled = Enabled; ColEnableChanged=true;}} //!< Enables or disables self-collision in the simulation.
	bool IsSelfColEnabled(void) {return SelfColEnabled;}
	...
	//Plasticity:
	void EnablePlasticity(bool Enabled) {PlasticityEnabled = Enabled;}
	bool IsPlasticityEnabled(void) {return PlasticityEnabled;}
	void EnableFailure(bool Enabled) {FailureEnabled = Enabled;}
	bool IsFailureEnabled(void) {return FailureEnabled;}
	int NumYielded(void);
	int NumBroken(void);
	float GetFloorHeight(float x, float y);
...
//	void SetViscousDamp(double ViscDampIn) {ViscousDamp = ViscDampIn;}
//	double ViscousDamp; //0->1. percent of velocity to shave off per second
	bool FluidDampEnabled; //do we want fluid-style damping?
	...
	int InputVoxSInd;
	int InputBondInd;
	CVXS_Voxel* InputVoxel(void);
	CVXS_Bond* InputBond(void);
	Vec3D<> PointerOffset;

	//Do some pseudo poisson's ratio constraints? (not useful yet)
	bool PoissonKickBackEnabled; //should the aspect ratio of the voxels affect the forces?

	//Impose angle and extension limits on the bonds (currently unstable)
	bool EnforceLatticeEnabled; //have limits for local bedning.compressive angles? (try again with RKS... a bit unstable with Euler du to rapidly changing stiffness

	void EnableMaxVelLimit(bool Enabled) {MaxVelLimitEnabled=Enabled;}
	bool IsMaxVelLimitEnabled(void) {return MaxVelLimitEnabled;}
	...
	bool IsEquilibriumEnabled(void){return EquilibriumModeEnabled;}
...
//	bool StopCondRqsEnergy(void) {return (StopConditionType==SC_CONST_MAXENERGY_10x50 || StopConditionType==SC_MIN_KE_10x50);}
...
//	sefertest.vxa
	...
	vfloat GetSumForceDir(CVX_FRegion* pRegion);

//	Nerual Circuitry
	float SensorValueArray[100];
	float NeuralOutputArray[100];

	float GetNeuralOutputOfIndex(int index){return NeuralOutputArray[index];}
	float GetSensorValueOfIndex(int index){return SensorValueArray[index];}
	void SetNeuralOutputOfIndex(int index, float value) {NeuralOutputArray[index]=value;}
	void SetSensorValueOfIndex(int index, float value) {SensorValueArray[index]=value;}
...
protected:
	...
	//Muscle Groups
	bool MuscleGroups; // Flag to denote if muscles are contracted through globabl temprature changes (false) or electric spikes (true)

	//Integration
	bool Integrate(IntegrationType Integrator = I_EULER);
	bool UpdateStats(std::string* pRetMessage = NULL); //returns false if simualtion diverged...
	IntegrationType CurIntegrator;
	//States for RK4 to use without re-initializing memory every time step...
	std::vector<CVXS_Voxel> OrigVoxArray, A, B, C, D;
	std::vector<CVXS_Bond> OrigBondArray;
	...
	bool SelfColEnabled; //do we want the overhead of watching for self-intersections? (for large displacement systems)
	...
	//Plasticity:
	bool PlasticityEnabled; //enables plastic deformation of non-linear materials
	bool FailureEnabled; //enables failure of materials (breaking of bonds)

	//Velocity limit
	bool MaxVelLimitEnabled;
	...
	CColor GetCurBondColor(int BondIndex);
	...
	char StatRqdToDraw(); //returns the stats bitfield that we need to calculate to draw the current view.
